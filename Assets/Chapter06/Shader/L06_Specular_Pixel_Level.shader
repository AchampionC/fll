...
Shader "FengLL/Chapter06/L06_Specular_Pixel_Level"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_TARGET // 告诉渲染器, 把用户的颜色存储到Render Target上, 这里将输出到默认的帧缓存中
            {
                ...
                float lambert = _DiffuseCol * ndotl;
                float phong = pow(max(0, dot(vDirWS, rlDirWS)), _SpecularPow);
                fixed3 finalRGB = ambient + _LightColor0.rgb * phong * _SpecularCol + lambert;
